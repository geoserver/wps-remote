...
class ComputationJobInput(object):
    ...
    def validate(self):
        ...
        if (len(res) == 1 and res[0]):
            ...
            raise TypeError("cannot validate value " + str(self._value) +
                            " for parameter " + self.get_name() + " with type " + self._type)
    ...
    def set_value(self, value):
        ...
        try:
            res = self.validate()
            if not res:
                raise TypeError("cannot set value " + str(self._value) + " for parameter " +
                                self.get_name() + " with type " + self._type)
        except BaseException:
            raise TypeError("cannot set value " + str(self._value) + " for parameter " +
                            self.get_name() + " with type " + self._type)

    def get_value(self):
        if isinstance(self._value_converted, list) and len(self._value_converted) == 1:
    ...
    def get_value_string(self):
        if isinstance(self._value, list) and len(self._value) == 1:
            if isinstance(self._value[0], datetime.datetime):
