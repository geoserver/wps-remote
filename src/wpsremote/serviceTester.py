...
class ServiceTester(sleekxmpp.ClientXMPP):
    ...
    def invokeService(self, service, origin):
        ...
        self.pid = hashlib.md5(str(time.time())).hexdigest(
        )[-5:] + '_' + hashlib.md5('%s%s' % (service, urllib.quote(msgPickledURLEnc))).hexdigest()

        # Send Request to the Service
        msgbody = 'topic=request&service=' + service + '&id=' + self.pid + \
                  '&message=' + msgPickledURLEnc + '&baseURL=localhost'
        print msgbody
        self.send_message(origin, mbody=msgbody)
