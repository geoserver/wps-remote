...
class CMREOAA_MainConfigFile(config_file_parameter.ConfigFileParameter):
    ...
    def update_file(self):
        # if target files exists open it and update, if doesnt exists load the
        # template, modify it and save to target (it is useful for multiple update
        # on the same file)
        if (self._filepath.exists()):
        ...
        if (self.get_name() == 'timeHorizon'):
            ...
            # todo: log "Cannot write input pararmeter " + name + " in config file " + str( self._config_file_path )
            pass
...
class Test(unittest.TestCase):
    ...
    def test_asset_config_object(self):
        ...
        paramsDef = [('assets',
                      {"type": "application/json",
                       ...
                       "max": 10,
                       "filepath": "d:\\tmp\\asset_$i.json",
                       "template_file_path": "$i",
                       "class": "config_file_parameter_cmre_oaa.CMREOAA_AssetConfigFile"})]
    ...
    def test_OAA_main_config_file(self):
        ...
        paramsDef = [('numberOfEvaluations',
                      {"type": "int",
                       "description": "number of optimizer iterations",
                       "max": 1,
                       "alias": "nEvaluations",
                       "class": "config_file_parameter_cmre_oaa.CMREOAA_MainConfigFile",
                       "filepath": "d:\\tmp\\oaa_config.json"}),
                     ('referenceDate',
                      {"type": "datetime",
                       "description": "start date of optimizer",
                       "max": 1,
                       "alias": "refDate",
                       "formatter": "%Y%m%d",
                       "class": "config_file_parameter_cmre_oaa.CMREOAA_MainConfigFile",
                     ...
                     ('timeHorizon',
                      {"type": "int",
                       "description": "Time horizon in seconds of the optimizer",
                       "max": 1,
                       "class": "config_file_parameter_cmre_oaa.CMREOAA_MainConfigFile",
                       "filepath": "d:\\tmp\\oaa_config.json"}),
                     ('executiontType',
                      {"type": "string",
                       "description": "etl execution type",
                       "max": 1,
                       "template": "value"})]
