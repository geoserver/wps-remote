...
class Resource(object):
    ...
    The method Resource.cleanup() looks fore pending resource files and if found
    might decide to kill process id found inside the file.
    """
    ...
    def clean_up(self):
        ...
        if dt > Resource.process_time_threshold:
            ...
            logger.debug("kill computational job, kill request handler process,  delete sandbox dir = " +
                         str(res))
            res = list(set(res))
            if (len(res) == 1 and res[0]):  # all success
                logger.info("all pending resource have been deallocated, remove resource file " +
                            str(self.filepath()))
                self.filepath().remove()
            else:
                logger.info("Cannot deallocate all pending resource, do not remove resource file " +
                            str(self.filepath()))
        else:
            logger.debug(str(self) + " is "+str(dt.total_seconds()) +
                         " seconds old and it is not yet ready to be cleaned up, wait for next round")

    def __str__(self):
        return "process " + str(self._processbot_pid) + " working on " + \
                                str(self._sandbox_path) + " from " + self.start_time_as_str()
