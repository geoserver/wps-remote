...
# -r .\\configs\\remote.config -s .\\configs\\RiskMaps\\service.config -p \
#    d:\\users\\cimino\\appdata\\local\\temp\\wps_params_q_245x.tmp process
...
# -r .\configs\remote.config -s .\configs\OAAonDemand\service.config -p \
#   d:\users\cimino\appdata\local\temp\wps_params_ullgiw.tmp process
...
class WPSAgent(object):
    ...
    def create_logger(logger_config_file, workdir, verbose):
        ...
        if workdir is not None:
            ...
            # to avoid problems with escape sequence (\ab is apparently a valid hex
            # number?!?!?), use always linux style path separator
...
class WPSAgentProcess(WPSAgent):
    ...
    His task is to call the proper external executable/scripts according to
    the service.config file (option -s in command line) and send back to the
    WPS logging and progress information and error information if something
    unexpected happens. All the output including the log file is generated
    in a sand box directory created with joint information from service.config
    and external process start-up information.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--remoteconfig", nargs='?', default="remote.config",
                        help="Config file containing connection information on the calling agent (e.g. XMPP server)")
    parser.add_argument("-s", "--serviceconfig", nargs='?', default="service.config",
                        help="Config file containing information concerning the local process to be invoked by WPS")
    parser.add_argument(
    "-l",
    "--logconf",
    nargs="?",
    default=None,
     help="Logger config file if not provided logger.conf in remoteconfig directory (runtype=service) or logger.conf \
         in serviceconfig directory (runtype=process) will be used")
