...
class XMPPRegisterMessage(object):
    ...
    def send(self):
        ...
        body = ''.join(['topic=register',
                        '&service=',
                        self.xmppChannel.get_fully_qualified_service_name(),
                        '&message=',
                        urllib.quote(json.dumps(service))])
...
class XMPPCompletedMessage(object):
    ...
    def send(self):
        try:
            ...
            body = ''.join(['topic=completed', '&id=', self.xmppChannel.id, "&baseURL=",
                            self._base_url, "&message=completed", "&", message])
            self.xmppChannel.xmpp.send_message(mto=self.originator, mbody=body, mtype='chat')
        except Exception:
            body = ''.join(['topic=error', '&id=', self.xmppChannel.id,
                            "&message=", "Critical error while encoding outuput!"])
            self.xmppChannel.xmpp.send_message(mto=self.originator, mbody=body, mtype='chat')
